...
// Max buffer size for FChangeLabel/FGetString/FWtzGetAccel
#define msocchTbStrLen 257
...
#define msotbdtSendTab             0x0010 // Send an msomdExitTab (rather than an
                                          ...
                                          // is hit.
                                          // Don't use 0x0020 because it will clash with msotbbtNoSetCursor
#define msotbdtStretchHorizontally 0x0200 // Dropdown combo will stretch horizontally to fill spare space on toolbar
...
#define msofuNone             0x00000000
#define msofuDrawBorder       0x00000001
// This turns off automatic inversion.  You MUST have UIS approval to turn off inversion.
// It affects accessibility so it must be used carefully.
#define msofuNoInversion      0x00000002
...
enum
{
	msocbvdbsClipImage = 0x00000010,		// Clip icon to rectangle
};
...
// Toolbar Clone flags 
 enum
	...
	msotbcfIgnorePopupFailure = 0x0020, // QFE 5392
	};
...
typedef enum
	...
	***********************************************/
	...
	msocbvcpxMenuInnerBorderX = 2,  // Orcas Visual Enhancements: Menu Improvements spec fig 5.3.2
	msocbvcpxMenuInnerBorderY = 2,  // Orcas Visual Enhancements: Menu Improvements spec fig 5.3.2
	...
	msocbvcpxDeprecated = 3,       // Can't remove this for backward compatability reasons
	...
	*****************************************/
	...
	// t-MPilat: Added these values so that they can be DPI adjusted once
	// and used many times.
	// These values must also be contiguous. If you change anything, check out
	// CbvCreateDimensionsBasedOnDPI to make sure nothing is wrong.
	
	// size of icon in button (always square)
	msocbvcpxSmallIconX,
	msocbvcpxLargeIconX,

	// size of actual button (not the same in x and y)
	msocbvcpxSmallButtonX,	
	msocbvcpxSmallButtonY,
	msocbvcpxLargeButtonX,
	msocbvcpxLargeButtonY,

	// swatch sizes
	msocbvcpxSmallColorSwatch,
	msocbvcpxLargeColorSwatch,

	// resizable dialog grippies
	msocbvcpxGrippies,

	msocbvcpxCmdBarOptionsCurveX,
	msocbvcpxCmdBarShadowY,
	msocbvcpxCmdBarPaddingX,
	msocbvcpxWPCaptionY,
	msocbvcpxWPNavBarY,
	msocbvcpxWPCaptionTitleY,
	msocbvcpxWPSeparatorX,
	msocbvcpxWPCurveY,
	msocbvcpxMenuWidthMax,
	msocbvcpxMenuInMenuWidthMax,
	msocbvcpxWPSizeDefaultX,
	msocbvcpxWPSizeDefaultY,
	msocbvcpxMenuItemPadding,	// Padding around text and icon in menus, used lots of places
	msocbvcpxWPCaptionTextX,
	msocbvcpxCBDockSeparatorY,
	...
	******************************************************/
	...
	msocbvcpxWPNavButtonX = msocbvcpxWPNavBarY,
	msocbvcpxWPNavButtonY = msocbvcpxWPNavBarY,
	
	
	msocbvcpxSmallIconY = msocbvcpxSmallIconX,
	msocbvcpxLargeIconY = msocbvcpxLargeIconX,
	msocbvcpxSmallIcon = msocbvcpxSmallIconX,
	msocbvcpxLargeIcon = msocbvcpxLargeIconX
...
#define msotbsfEnableHorizWP        0x00040000 // enable horizontal WorkPanes
#define msotbsfShowHomeBtn          0x00080000 // show the WorkPane home button
#define msotbsfDrawSeparatorLine    0x00200000  // TRUE to draw two pixel seperator line below top dock area
#define msotbsfPreventFlicker		0x00400000	// TRUE to use off screen drawing to "prevent flicker"
...
enum
	...
	msogccFullExtent             = 0x2000, // draw the gcc the full width of the toolbar
	};	
...
#define msotbtrTbInvisible (msotbtrTbBasic |\
							msotbtrNoChangeVisible|\
							msotbtrNoTbContextMenu)
...
#define msotbidReviewCommentsExtra (msotbidReviewComments | msotbidQuickCustomize)
#define msotbidReadingExtra (msotbidReadingMode | msotbidQuickCustomize)
...
#define msotbtr2TransparentGCC  (1 << 6) // Draw this GCC transparently
#define msotbtr2GccOnTabSheet   (1 << 7) // This GCC will be in a tab sheet (and hence needs a gradient bg when themed)
// Available:        (1 << 8) // Used to be for hi-res icons
#if 0 //$[VSMSO]
#else //$[VSMSO]
#define msotbtr2VSToolWindowToolbar (1 << 9) // This toolbar will have an options menu, only if necessary (if truncated) with no customization, just the truncated controls
#endif //$[VSMSO]
...
// THE OFFICE SCALING DEFAULT. 
// Consult with PaulCole or DMorton before touching this value.
//#define msohdpiDefaultScaleMode		msohdpiScaleNearestHalf
...
enum
	...
	msotbsaveCustomImage    // VS-extension for saving custom bitmaps from a button
	};
...
enum
	...
	msotbloadCustomImage            // VS-extension for loading custom bitmaps into a button
	};
...
// NOTE: This structure is mirrored in Netui, in netui\platform\inc\NetUILibc.h
//       If you make any changes here, make them there as well. And yes, I am aware that
//       this is terribly bad form.
#ifndef MSO_NO_INTERFACES
#define MSOSTCR_DEFINED
...
#define msotuoFull               0x000  // Normal update (redraw if necc, no interrupts)
#define msotuoNoDraw             0x001  // Don't redraw new state
#define msotuoInterrupt          0x002  // Update is interruptable if input event occurs
#define msotuoMetricChange       0x004  // System metrics have changed, some controls may need to resize
#define msotuoReverseUpdateOrder 0x008  // Causes the sorting order for Toolbar update to be reversed.
                                       //    Used only with TBS::Activate currently.
#define msotuoMainMenuOnly       0x010  // main menu bar only
#define msotuoPaletteChange      0x020  // Palette has changed, some controls may need to redraw
#define msotuoForceDockReflow    0x040  // Force docks to collapse unneccesary space (exiting customize)
#define msotuoUpdateHidden       0x080  // Force an update even though the toolbar is hidden (for GCCs in autolayout)
#define msotuoNoContentUpdate    0x100  // Don't update content.  (See OfficeNet bug 134226.)
#define msotuoNoCloseOnUpdate    0x200  // If a menu item is dynamically updated while mouse is hovering, KillAllTracking 
					  // is normally called.  This flag will prevent that so the menu doesn't close (OfficeNet 140884)
#define msotuoForceTextUpdate    0x400  // Force update the labels of items currently displaying text (HideUnderlines support)
...
#ifndef MSO_NO_INTERFACES
/* moved into a separate include file in order to enable inclusion from
** both NetUI and MSO
*/
#include "msoceo.h"
#endif // MSO_NO_INTERFACES
...
#define msoppMyUISucksSoINeedAccelerators 0x00000400
#define msoppShowUnderlines   0x00000800   // Force underlines to be shown on this popup
#define msoppSupportsTypeAhead 0x00001000   // Supports type ahead.  Preclueds all other accelerators and does not select upon focus any item.
...
enum
	...
	msotbstrCanonicalText,			// Non-localized canonical English text.
	};
...
enum
	...
	/* Sent to the currently visible WorkPane when the app receives a
	   message which the system sends only to top-level windows.  Note that
	   the WM_PALETTECHANGED and WM_QUERYNEWPALETTE messages should fall into
	   this category, but in order to maintain backwards compatibility, they
	   need to retain their own messages.  (The msotbwpcWM_PALETTECHANGED and
	   msotbwpcWM_QUERYNEWPALETTE messages were available in Office10, and the
	   msotbwpcWM message wasn't added until Office.Net.)
	   	in:  param1 = HWND           HWND of dialog
	   	     param2 = the MSG to be sent */
	msotbwpcWM,
	};
...
typedef enum
	{
	/* NetUI dialog will be laoded from a resouce ID */
	msoNetUIDlgFromRes,

#ifdef DEBUG
	/* NetUI dialog will be loaded from a file */
	msoNetUIDlgFromFile,
#endif

	/* 
	enum values defined beyond this point a flags, we use low 16 bit to
	store pane type and high 16 bit to store netpane flags.
	*/

	msoNetUIDlgTypeMask = 0x0000FFFF,

	/* Keep list of named parsed NetUI flags in the NUIDocument object */
	msoNetUIDlgKeepParsedStyles = 0x00010000,
	// Do not apply global default style sheet
	msoNetUIDlgsDoNotApplyGlobalDefaultStyle = 0x00020000,
	} MSONETUIDLG;
...
typedef HRESULT (MSOPRIVCALLTYPE * MSOPFNNPCALLBACK)
                               (MSONPCALLBACKTYPE msonpct, int idWP, void * pv);

/* NetUI WorkPane struct - filled out partially by the caller, partially by
		creation of the dialog */
typedef struct _msowpnetui
	{
	/* This section filled out by caller before passing to TBS::FShowWorkPane */

	// position and size to resize the hwnd.
	RECT rc;

	// toolbar set this wp lives in
	interface IMsoToolbarSet * pitbs;

	// dialog specific info, dialog needs to handle manually for SaveBe
	void *pv; 

	// callback for registering custom events, eventhandlers and controls
	MSOPFNNPCALLBACK pfnNPCallback;

	// dialog type, and info
	MSONETUIDLG msonuidlgType;
	HINSTANCE   hinst;     // hinst where dialog lives
	HINSTANCE   hinstIntl; // hinst where dialog intl resources live
	union
	{
#ifdef DEBUG
		WCHAR * wzFile;        // path to netui dialog
#endif
		struct
		{
		UINT        rcid;      // resource ID for the dialog
		};
	};
	
	
	/* This section filled out by pfn when msotbwpcCreateWindow is called */
	void * pNUIDoc;
	void * pnp;
	int idWP;
	} MSOWPNETUI;
...
#define msoomWPHasFocus            1009
#define msoomWPClientChangedSize   1010
#define msoomWPGetCurrentID        1011
...
// Gradients for MsoCbvDrawTBDockBackground
typedef enum
{
	msocbvgrMin = 0,
	msocbvgrCBVertical = 0,
	msocbvgrCBMainMenuHorizontal = 1,
	msocbvgrCBMenuTitleBkgd = 2,
	msocbvgrCBMenuIconBkgd = 3,
	msocbvgrCBOptions = 4,
	msocbvgrCBOptionsHorizontal = 5,
	msocbvgrCBHorizontal = 6,
	msocbvgrCBMenuExpander = 7,
	msocbvgrCBSelected = 8,
	msocbvgrCBMouseOver = 9,
	msocbvgrCBMouseDown = 10,
	msocbvgrCBSelectedMouseOver = 11,
	msocbvgrCBSelectedHorizontal = 12,
	msocbvgrCBMouseOverHorizontal = 13,
	msocbvgrCBMouseDownHorizontal = 14,
	msocbvgrCBSelectedMouseOverHorizontal = 15,
	msocbvgrCBMenuCtlMouseOver = 16,
	msocbvgrMax = 17,
} CBVGRADIENT;
...
DECLARE_INTERFACE_(IMsoControlUser, ISimpleUnknown)
{
	...
	MSOMETHOD_(BOOL, FChangeLabel) (THIS_ interface IMsoControl *pic, 
												void **ppv, const WCHAR *wtzOld, _Out_cap_(msocchTbStrLen) WCHAR *wtzNew) 
												PURE;
	...
	MSOMETHOD_(BOOL, FGetString) (THIS_ interface IMsoControl *pic, 
			void **ppv, int tbstr, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FGetQuickTip)(THIS_ interface IMsoControl *pic, 
							void **ppv, _Out_cap_((MAX_PATH+1)) WCHAR *wtzHelpFile, int *pidHelpContext) PURE;
...
DECLARE_INTERFACE_(IMsoPaneUser, IMsoControlUser)
{
	...
	MSOMETHOD_(BOOL, FGetAlternateControl) (THIS_ interface IMsoControl *pic, 
												...
												PURE;
	MSOMETHOD_(BOOL, FChangeLabel) (THIS_ interface IMsoControl *pic, 
												void **ppv, const WCHAR *wtzOld, _Out_cap_(msocchTbStrLen) WCHAR *wtzNew) PURE;
	MSOMETHOD_(BOOL, FNotifyAction) (THIS_ interface IMsoControl *pic, 
												void **ppv, int tba) PURE;
	MSOMETHOD_(int, TbcuOleUsage)(THIS_ interface IMsoControl *pic, 
												void **ppv) PURE;
...
DECLARE_INTERFACE_(IMsoDropdownUser, IMsoControlUser)
{
	...
	MSOMETHOD_(VOID, GetText) (THIS_ interface IMsoControl *pic, 
												void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(VOID, GetDropItem) (THIS_ interface IMsoControl *pic, 
							void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz, int iItem, int *ptcid) PURE;
	...
	MSOMETHOD_(VOID, EnterText) (THIS_ interface IMsoControl *pic, 
												void **ppv, _Out_opt_cap_(msocchTbStrLen) WCHAR *wtz, int mdExit) PURE;
	...
	MSOMETHOD_(void, UpdateStatusBar) (THIS_ interface IMsoControl *pic, 
												void **ppv, int iSelection, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
};
#endif // MSO_NO_INTERFACES
...
DECLARE_INTERFACE_(IMsoWysiwygDropdownUser, IMsoDropdownUser)
{
	...
	MSOMETHOD_(VOID, Pick) (THIS_ interface IMsoControl *pic, 
												void **ppv, int i, int mdExit) PURE;
	MSOMETHOD_(VOID, EnterText) (THIS_ interface IMsoControl *pic, 
												void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz, int mdExit) PURE;
	MSOMETHOD_(int, CDropdownLines) (THIS_ interface IMsoControl *pic, 
												void **ppv) PURE;
	MSOMETHOD_(int, DxDropdown) (THIS_ interface IMsoControl *pic, 
												void **ppv, int dxOptimal) PURE;
...
DECLARE_INTERFACE_(IMsoOCXDropUser, IMsoControlUser)
{
	...
	MSOMETHOD_(void, Pick) (THIS_ interface IMsoControl *pic,void **ppv, _Out_opt_cap_(msocchTbStrLen) WCHAR *wtzClsid, int mdExit) PURE; 
...
DECLARE_INTERFACE_(IMsoOwnerDropdownUser, IMsoDropdownUser)
{
	...
	MSOMETHOD_(VOID, GetText) (THIS_ interface IMsoControl *pic, 
												void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	MSOMETHOD_(VOID, GetDropItem) (THIS_ interface IMsoControl *pic, 
												void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz, int i, int *ptcidIcon) PURE;
	MSOMETHOD_(int, CItems) (THIS_ interface IMsoControl *pic, 
												void **ppv, int *pcMRU, int *pfFullSel) PURE;
...
DECLARE_INTERFACE_(IMsoExpandingGridUser, IMsoButtonUser)
{
   ...
   MSODEBUGMETHOD
	...
	/* App should fill wtz with text to draw in the status bar. (x, y) is the 
		...
		all. */
	MSOMETHOD_(void, UpdateStatusBar) (THIS_ interface IMsoControl *pic, 
												void **ppv, int x, int y, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
...
DECLARE_INTERFACE_(IMsoSwatchUser, IMsoControlUser)
{
	...
	MSOMETHOD_(BOOL, FGetStringEx) (THIS_ interface IMsoControl *pic, 
			void **ppv, int tbstr, _Out_cap_(msocchTbStrLen) WCHAR *wtz, int x, int y) PURE;
...
DECLARE_INTERFACE_(IMsoControl, ISimpleUnknown)
{
	...
	MSOMETHOD_(void, GetLabel) (THIS_ _Out_cap_(msocchTbStrLen) WCHAR *wtzLabel, BOOL fCurrent) PURE;
	...
	MSOMETHOD_(BOOL, FGetHelpFile) (THIS_ _Out_cap_((MAX_PATH+1)) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FSetHelpFile) (_In_opt_z_ THIS_ WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FGetString) (THIS_ int tbstr, BOOL fGenerate, _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FSetString) (THIS_ int tbstr, const WCHAR *wtz) PURE;
	...
	/* Set the OnAction macro reference string for the control to a copy of wtz
		...
		Return FALSE if failure. */
	MSOMETHOD_(BOOL, FSetOnAction) (THIS_ const WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FGetOnAction) (THIS_ _Out_opt_cap_(msocchTbStrLen) WCHAR *wtz) PURE;

	/* If the control has no accelerator, return FALSE, otherwise return TRUE
		and copy the accelerator string in wtz (limit is 255). */
	MSOMETHOD_(BOOL, FGetAccelerator) (THIS_ _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FSetAccelerator) (THIS_ const WCHAR *wtz) PURE;
};
#endif // MSO_NO_INTERFACES
...
DECLARE_INTERFACE_(IMsoToolbarUser, ISimpleUnknown)
{
	...
	MSOMETHOD_(void, GetContext) (THIS_ void **ppv, _Out_cap_(msocchTbStrLen) WCHAR *wtz, interface IMsoToolbar *pitb) PURE;

	// Set new context, or return FALSE if invalid context
	MSOMETHOD_(BOOL, FSetContext) (THIS_ void **ppv, const WCHAR *wtz, interface IMsoToolbar *pitb) PURE;
	...
	*/
	MSOMETHOD_(BOOL, FGetNameEnu) (THIS_ interface IMsoToolbar *pitb, void **ppv, 
										_Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
};
#endif // MSO_NO_INTERFACES
...
DECLARE_INTERFACE_(IMsoToolbar, IMsoControlContainer)
{
	...
	MSOMETHOD_(void, GetName) (THIS_ _Out_cap_(msocchTbStrLen) WCHAR *wtz) PURE;
	...
	MSOMETHOD_(BOOL, FPopup) (THIS_ int x, int y, int pp, const RECT * prc) PURE;
	...
	*/
	...
	// Sets/Gets one of the private flag members
...
DECLARE_INTERFACE_(IMsoToolbarSetUser, ISimpleUnknown)
{
	...
	MSOMETHOD_(BOOL, FCreateCustomToolbar) (THIS_ 
			...
			MSOTBP *ptbp, const WCHAR *wtzName, interface IMsoToolbar **ppitb) PURE;
	...
	MSOMETHOD_(interface IMsoToolbarUser *, PitbuGetToolbarUser) (THIS_
			interface IMsoToolbarSet *pitbs, void **ppv, 
			int tbid, const WCHAR *wtzName) PURE;
	...
	*/
	MSOMETHOD_(BOOL, FIsTbNameDuplicate) (THIS_ interface IMsoToolbarSet *pitbs,
														void **ppv, const WCHAR *wtzName) PURE;
	...
	/* This is called during control load to allow the host app to adjust/remap
		dynamic tcid values, for portability reasons. */
	MSOMETHOD_(BOOL, FRemapDynamicTcid) (THIS_ interface IMsoToolbarSet *pitbs,
			void *ppv, int tct, int tbct, IMsoControlSite *pics, int *ptcid);
};
#endif // MSO_NO_INTERFACES
...
DECLARE_INTERFACE_(IMsoToolbarSet, ISimpleUnknown)
{
	...
	*/
	...
	MSOMETHOD_(BOOL, FDoComplexContextMenu) (THIS_ POINT pt, int tbid, int cctl,
			MSOCTCR *rgctcr, BOOL fKeyboard, int *pictl, const RECT * prcAvoid, int pp) PURE;
...
/* Gets "hide underline/mnemonic" mode. */
MSOAPI_(BOOL) MsoFGetHideUnderlines();

/* Gets "long menu" mode. */
MSOAPIX_(BOOL) MsoFGetLongMenus();
...
/* Same as MsoFCreateButtonEx, but allows passing of a transparent color for
    the custom bitmap. */
MSOAPI_(BOOL) MsoFCreateButtonEx2(HMSOINST hinst, int tcid, 
										   int tbct, int tbidMenu, void *pvClient, 
										   interface IMsoControlSite *pics,
										   interface IMsoControlUser *picu,
										   interface IMsoControl **ppic,
...
/*	This is the message proc to forward keystrokes to for a win32 gcc.
	Note:  This is unsupported by the command bars team, and is only currently 
	used for Outlook Dialogs.  USE AT YOUR OWN RISK.  (You will get no help) */
MSOAPI_(LRESULT) MsoWin32GCCMsgProc(IMsoToolbar *pitb, MSG *pmsg);
...
#if 0 //$[VSMSO]
// The following set of APIs were added to support OfficeLite, but can
// really be used for any Office User, not just the Lite ones.

/* Allows the Office client to register which set of TCID's should
   not be allowed in this SKU.  This controls can be created, but
	 Office will never show them.  The pointer should point to a
	 sorted array of TCID's, and the count should be how many there
	 are.  It is expected that this function is called only once
	 during the life of the process.  Calling it a second time with
	 a different set of TCID's will not result in MSO going and displaying
	 all the controls that were formerly hidden based on the original array.
   Returns TRUE if parameters are well formed */
MSOAPI_(BOOL) MsoFSetTcidsRemovedFromSKU(_Out_cap_(ctcid) int *prgtcid, int ctcid);

/* Looks up tcid in the registered list of removed from SKU controls.
   Returns TRUE if it finds it, FALSE if not, or there isn't a list even
	 registered */
MSOAPI_(BOOL) MsoFTcidRemovedFromSKU(int tcid);
#endif //$[VSMSO]
...
MSOAPI_(BOOL) MsoFSetButtonFaceEx(IMsoControl *pic, HDC hdc, HBITMAP hFace, HBITMAP hMask, COLORREF crTransparent, BOOL fDib);
...
MSOAPI_(int) MsoFRestoreCtrl(IMsoToolbar *pitb, const WCHAR *wtzName);
...
MSOAPI_(void) MsoGetLabelFromTcid(int tct, int tcid, _Out_z_cap_(cchLabel) WCHAR *wtzLabel, int cchLabel);

/* Return button face bitmap for a given TCID */
MSOAPI_(BOOL) MsoFGetBitmapFromTcid(int tcid, HBITMAP *phBitmap);
...
MSOAPI_(void) MsoGetTooltipOverrideFromTcid(int tct, int tcid, _Out_z_cap_(cchTip) WCHAR *wtzTip, int cchTip);

/* Return the build-in resource label text associated with a given command 
	bar and return it in wtzLabel. */
MSOAPI_(void) MsoGetLabelFromTbid(int tbid, _Out_z_cap_(cchMax) WCHAR *wtzLabel, int cchMax, BOOL fLocalized);
...
MSOAPI_(WCHAR *) MsoPwchStripAmpersandsWtz(_Inout_z_ WCHAR *wtz);
...
MSOAPI_(void) MsoStripTooltipWtz(_Inout_z_ WCHAR *wtz);
...
*/
MSOAPI_(WCHAR *) MsoPwchStripWtz(_Inout_z_ WCHAR *wtz, int rgfStrip);
...
MSOAPI_(void) MsoGetScreenRect(RECT *prc, BOOL fWorkspace, const POINT * pptNear);
...
MSOAPI_(BOOL) MsoFForceRectOnScreenEx(RECT * prc, BOOL fWorkspace, BOOL fForceEntire);
...
/* Set the values of phbstrip and piBtn which point to the strip and
	...
	returned.
	*/
MSOAPI_(BOOL) MsoFFindBtnFaceEx(int id, int* msobf, HBSTRIP *phbstrip, int *piBtn);

/* Is the given button face id blank. Either it is msotcidNoIcon or it maps
   to the same blank icon */
MSOAPI_(BOOL) MsoFIsBtnFaceBlank(int id);
...
MSOAPI_(BOOL) MsoFShowTooltip(const WCHAR *wtz, int tt);

/* Same as above, but allows you to use a POINT to position the tooltip
   Also, calling this function disables hiding the tip on mouse up. */
MSOAPI_(BOOL) MsoFShowTooltipInplace(const WCHAR *wtz, int tt, POINT pt);
...
/* NOTE: This API is used from Netui's netui\platform\inc\NetUILibc.h. Any changes
         you make here, make there as well */
...
MSOAPI_(BOOL) MsoFDoComplexContextMenu(HMSOINST hinst, POINT pt, int tbid, int cctl,
		MSOCTCR *rgctcr, BOOL fKeyboard, int *pictl, const RECT * prcAvoid, int pp);
...
MSOAPI_(BOOL) MsoFCreateComplexMenu(HMSOINST hinst, int tbid, const WCHAR *wtzName, int cctl, MSOCTCR *rgctcr, 
...
// Can be used to 'double' special chars like &.
MSOAPI_(BOOL) MsoFExpandSpecialCharsWtz(const WCHAR *wtzSrc, _Out_z_cap_(cchDest) WCHAR *wtzDest, 
		int cchDest, WCHAR wchSpecial);

#define MsoFExpandAmpersandsWtz(wtzSrc, wtzDest, cchDest) \
		MsoFExpandSpecialCharsWtz(wtzSrc, wtzDest, cchDest, chAmpersand)

/*-----------------------------------------------------------------------------
	High-DPI Goodies
	These API's are for scaling things like images and icons to display well on 
	high-dpi devices like laptops and tablets.
---------------------------------------------------------------- t-MPilat ---*/
/* Gets the DPI-adjusted width */
#define MsoScaleForDPI(v) MsoScaleForDPILinear(v)
MSOAPI_(int) MsoScaleForDPILinear(int value);
MSOAPI_(int) MsoScaleForDPINearestHalfStep(int value);

/* TRUE if things will be scaled up from 96 dpi (or, dpi>96) */
MSOAPI_(BOOL) MsoFScaling();

/* TRUE if things are scaling by an integer multiple */
MSOAPI_(BOOL) MsoFScalingInteger();

/* Retrieves the average width and the height, in pixels, of the characters
   in the font used by the dialog. */
MSOAPI_(LONG) SDMGetDialogBaseUnits(void);
...
/* Get a system color from MSO's cache. */
MSOAPI_(COLORREF) MsoCrSysColorGet(int color);

/* Fill rect with cr, restore the hdc background color when done. */
MSOAPI_(void) MsoCbvFillRect(HDC hdc, RECT *prc, COLORREF cr);

/* Fill rect with cr, or with transparent bkgnd design if cr == msocbvcrNetLookBkgnd */
MSOAPI_(void) MsoCbvFillRectEx(HDC hdc, RECT *prc, COLORREF cr, MSOCEO* pceo, int tbbsx, int cpxgrOffset);

/* Fill rect with cr, don't replace the old hdc background color */
MSOAPI_(void) MsoCbvFillRectFast(HDC hdc, RECT *prc, COLORREF cr);

/* Fill rect with cr, don't set or replace the hdc background color */
MSOAPI_(void) MsoCbvFillRectSuperFast(HDC hdc, RECT *prc);
...
MSOAPIX_(int) MsoCpxCbvGetAccurate(const MSOCEO *pceo, MSOCBVCPX msocbvcpx);

/* Returns TRUE if we should be using the new visuals for commandbars, given
   the pceo and tbbsx. */
MSOAPIX_(BOOL) MsoFUseCbv(const MSOCEO *pceo, int tbbsx);

/* Returns TRUE if the current screen color depth is 256 colors, or lower. */
MSOAPI_(BOOL) MsoFCbvLowColor();
...
#if 0 //$[VSMSO]
/* Getter/Setter for TBS.m_fUsingWPAltCoord */
MSOAPI_(BOOL) MsoFUsingWPAltCoord(IMsoToolbarSet* pitbs);
MSOAPI_(void) MsoSetUsingWPAltCoord(IMsoToolbarSet* pitbs, BOOL fAltCoord);
#endif //$[VSMSO]
...
MSOAPI_(void) MsoRenderSolidPaintRectangle(HDC hdc, const RECT *prc, int cs, COLORREF cr);
...
MSOAPI_(void) MsoCbvDrawButton(MSOCTLENV *pce, int tcid, IMsoButtonUser *pibu, 
						...
						int tbbsx, const WCHAR *wtz, int ftReq, int tbbsx2);

/* Includes support for drawing gradients */
MSOAPI_(void) MsoCbvDrawButtonEx(MSOCTLENV *pce, int tcid, IMsoButtonUser *pibu, 
						void **ppvPibu, HANDLE hdibCustom, HANDLE hmaskCustom, 
						int tbbsx, const WCHAR *wtz, int ftReq, int tbbsx2, int cpxgrOffset);
...
* This API moved into msoceo.h, so that it can also be included from NETUI*/
...
MSOAPI_(void) MsoDrawOneBorderWithTheme(HDC hdc, RECT *prc, COLORREF crUpper, COLORREF crLower);
...
// These functions allow for standard handling of NetUI workpanes.
//		If there is no need for overrides, use the first function to get a pfn
//			to pass into TBS::FShowWorkPane
//		If there is need for overrides, create your own pfn and at the end of
//			all un-handled messages, call the second function (think: DefWindowProc)
MSOAPI_(BOOL) MsoFCallNetUIWorkPaneCallback(int idWP, int tbwpc, void * pwpInfo, 
				void * param1, void * param2);

MSOAPI_(MSOPFNWPCALLBACK) MsoPfnGetNetUIWorkPaneCallback(void);


MSOAPI_(void) MsoIdealAutoLayoutCurDlgWP();
MSOAPI_(BOOL) MsoFCloseWorkpane(HWND hwndWP);
#endif //$[VSMSO]
...
MSOAPI_(BOOL) MsoFPassMsgToComponent(const MSG *pmsg);
MSOAPI_(void) MsoSetUnicodeMessageLoop(BOOL fUnicode);

MSOAPI_(void) MsoTBSSetFocusToWorkpane(IMsoToolbarSet *pitbs);


// Toolbar Focus State defines, to be used with
enum
	{
	msotbfsNone     = 0x0000,
	msotbfsToolbar  = 0x0001, // normal toolbar, not a GCC, not the workpane, not a wp lite, not a popup
	msotbfsWorkPane = 0x0002, // tbid is msotbidWorkPane, accept no substitutes
	msotbfsPane     = 0x0004, // any "pane" style toolbars (including wp lites) that are not the workpane itself
	msotbfsGCC      = 0x0008,
	msotbfsPopup    = 0x0010,
	};

MSOAPI_(int) MsoGetToolbarFocusState();

// Used to avoid setting focus for toolbar related reasons while an app is deactivating
MSOAPI_(BOOL) MsoFInDeactivateApp();
...
MSOAPI_(void) MsoCbvDrawTBDockBackground(HDC hdc, RECT* prect, int nOffset, CBVGRADIENT grType);
MSOAPI_(void) MsoCbvDrawSmallAntiAliasedCurve(HDC hdc, int x, int y, int cbvtbCurve, COLORREF crInside, COLORREF crOutside);

#define CIC_NUM float // slightly faster than using double.
MSOAPI_(BOOL) MsoFCbvEnoughContrast(COLORREF crLocked, COLORREF crMoveable, CIC_NUM dMinContrast, BOOL fDarkness);
MSOAPI_(void) MsoCbvIncreaseContrast(COLORREF crLocked, COLORREF *pcrMoveable, CIC_NUM cMinContrast);
MSOAPI_(void) MsoCbvDecreaseContrast(COLORREF crLocked, COLORREF *pcrMoveable, CIC_NUM cMaxContrast);
MSOAPI_(BOOL) MsoFCbvEnoughContrastCached(COLORREF crLocked, COLORREF crMoveable,
                              CIC_NUM dMinContrast, BOOL fDarkness);

MSOAPI_(BOOL) MsoFPortraitMode();

MSOAPI_(void) MsoCbvDrawIconEx(HDC hdc, MSOCEO *pceo, int tcid, HANDLE hdibCustom,
			HANDLE hmaskCustom, HBSTRIP hbstrip, int iBtnFace, RECT *prcDest, SIZE* pdxySrc, COLORREF crBkgd, 
			int tbbsx, UINT fuFlags);
...
MSOAPI_(HANDLE) TB_CreateMask(DWORD  RGBBkgd, BITMAPINFO *pbmiColor, BOOL fAlphaBlend);
MSOAPI_(HANDLE) TB_CreateDisable(BITMAPINFO *pbmiColor, BITMAPINFO *pbmiMask);
MSOAPI_(int) TB_GetSize(const BITMAPINFO *pbmi);
...
HDC msoDebugGetDC(HWND hwnd, _In_z_ char *szfile, int iline);
int msoDebugReleaseDC(HWND hwnd, HDC hdc, _In_z_ char *szfile, int iline);
