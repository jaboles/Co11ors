...
MSOAPIX_(VOID) MsoSdmChangeTheme(HWND hwnd);
...
#define PcabLockCab(hcab, sb) (*(hcab))
...
MSOAPI_(VOID)	GetCabWtz(HCAB, _Out_z_cap_(cchMac) WTZ_CAB wtz, UINT_SDM cchMac, UINT_SDM);
MSOAPI_(BOOL_SDM)	FSetCabWz(HCAB, CONST_WZ_CAB, UINT_SDM, SB_SDM);
MSOAPI_(UINT_SDM)	CchOfCabWz(HCAB, UINT_SDM);
MSOAPI_(VOID)	GetCabWz(HCAB, _Out_z_cap_(cchMac) WZ_CAB wz, UINT_SDM cchMac, UINT_SDM);
MSOAPI_(BOOL_SDM)	FSetCabRgb(HCAB,  _In_bytecount_(cb) CONST_RGB_CAB, UINT_SDM cb, UINT_SDM, SB_SDM);
MSOAPI_(VOID)	GetCabWt(HCAB, _Out_z_cap_(cchMac) WTZ_CAB wt, UINT_SDM cchMac, UINT_SDM);
MSOAPI_(BOOL_SDM)	GetCabRgb(HCAB,  _Out_bytecap_(cbMac) RGB_CAB, UINT_SDM cbMac, UINT_SDM, SB_SDM);
...
MSOAPI_(BOOL_SDM) MsoFSetCabHandleRgb(PCABH, _In_bytecount_(cb) CONST_RGB_CAB, UINT_SDM cb, SB_SDM);
MSOAPI_(VOID)	  MsoGetCabHandleWtz(PCABH, _Out_z_cap_(cchMac) WTZ_CAB wtz, UINT_SDM cchMac);
MSOAPI_(VOID)	  MsoGetCabHandleWz(PCABH, _Out_z_cap_(cchMac) WZ_CAB wz, UINT_SDM cchMac);
MSOAPI_(VOID)	  MsoGetCabHandleWt(PCABH, _Out_z_cap_(cchMac) WT_CAB wt, UINT_SDM cchMac);
MSOAPI_(BOOL_SDM) MsoFGetCabHandleRgb(PCABH, _Out_bytecap_(cbMac) RGB_CAB, UINT_SDM cbMac, SB_SDM);

// Generic Dialog Control
MSOAPI_(void) 	MsoInitDli(DLI *, HWND, FDLG, UINT_SDM);
MSOAPI_(short) 	MsoCbOfDlt(long, HINSTANCE);
MSOAPI_(BOOL) 	MsoFBltDlt(long, void *pdltDest, DWORD cbdltDest, HINSTANCE);
...
MSOAPI_(void)	MsoConvertRecToRc(const REC *, RECT *);
...
#define msoGrippieSize MsoScaleForDPI(11)
...
// Why are they only using 1 byte at a time?
// For Word's dialog in a tab sheet. For example, Tools | Options. Need to be able
// to identify these in LSdmWP so we can draw the proper background or no background
// under Whistler theming. Tab controls have a textured background that is different
// from the dialog background.
#define msogrfHdlgStartTabSheetDialog 0x00001000
												

MSOAPI_(TMC) TmcDoDlgDliEx(struct _dlgt **, HCAB hcab, DLI *pdli, DWORD grfHdlgStart);
MSOAPIX_(TMC)	TmcDoDlg(struct _dlgt * *, HCAB, BYTE *);
MSOAPI_(TMC)	TmcDoDlgDli(struct _dlgt * *, HCAB, DLI *);
MSOAPI_(HDLG)	HdlgStartDlg(struct _dlgt * *, HCAB, DLI *);
MSOAPI_(HDLG) HdlgStartDlgEx(struct _dlgt * *, HCAB, DLI *, DWORD);
...
MSOAPI_(VOID)	GetTmcLargeVal(TMC, VOID * pval, UINT_SDM cbMac);
...
MSOAPI_(VOID)	GetTmcText(TMC, _Out_z_cap_(cchMac) WCHAR * wz, UINT_SDM cchMac);
MSOAPI_(UINT_SDM)	CchGetTmcText(TMC, _Out_z_cap_(cchMac) WCHAR * wz, UINT_SDM cchMac);
...
MSOAPI_(PFN_FILTERMSG) MsoPfnSetPfnFilterMsg(PFN_FILTERMSG pfnNewFilter);
...
// Needed for Windows XP theming
MSOAPI_(void) MsoSetTabSwitcherBorder(TMC tmc);
...
MSOAPI_(VOID)	ShowTabSheetDlg(BOOL_SDM);
...
MSOAPI_(VOID)	SetTabOrder(const TMC * rgtmc, UINT_SDM ctmc);
MSOAPI_(int) 	SwapTabOrder(TMC tmc1, TMC tmc2);
MSOAPI_(VOID)	UpdateWindowDlg(HDLG);

MSOAPI_(VOID)	GetListBoxEntry(TMC, ILBE_SDM, _Out_z_cap_(cchMac) WCHAR * wz, UINT_SDM cchMac);
MSOAPI_(UINT_SDM)	CchGetListBoxEntry(TMC, ILBE_SDM, _Out_z_cap_(cchMac) WCHAR * wz, UINT_SDM cchMac);
...
// Needed for theming Word's checkbox in a groupbox/groupline.
MSOAPI_(int) MsolCheckboxTextMarginGet();
...
MSOAPI_(VOID) DrawSDMFocusRect(HDC hdc, const RECT *pRect, COLORREF crFore, COLORREF crBkg);
...
UCBK_SDM SDM_CALLBACK MsoDrawAsstBtn(TMM tmm, RDS *prds, UINT_SDM, FTMS ftmsNew,
									 FTMS ftmsOld, TMC tmc, UCBK_SDM wParam);

MSOAPI_(UCBK_SDM) SDM_CALLBACK	WRenderButton(TMM, RDS *, UINT_SDM,  FTMS, FTMS, TMC, UCBK_SDM);
UCBK_SDM SDM_CALLBACK	WRenderPush(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
UCBK_SDM SDM_CALLBACK	WRenderStaticText(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
...
#ifdef DEBUG
	MSOAPI_(UCBK_SDM) SDM_CALLBACK	WRenderRadio(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
	MSOAPI_(UCBK_SDM) SDM_CALLBACK	WRenderCheck(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
	MSOAPI_(UCBK_SDM) SDM_CALLBACK	WRenderGroup(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
#else
	UCBK_SDM SDM_CALLBACK	WRenderRadio(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
	UCBK_SDM SDM_CALLBACK	WRenderCheck(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
	UCBK_SDM SDM_CALLBACK	WRenderGroup(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
#endif

MSOAPIDBG_(UCBK_SDM) SDM_CALLBACK	WRenderDropIcon(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
...
MSOAPI_(UCBK_SDM) SDM_CALLBACK MsoWRenderAsTbButton(
	...
	const WCHAR*wtzLabel,			// Button text
