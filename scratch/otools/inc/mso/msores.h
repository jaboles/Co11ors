...
#if DEBUG
	MSOAPIXX_(BOOL) MsoFValidBStrip(const struct BSTRIP* hbstrip);
#else
...
MSOAPI_(BOOL) MsoFDrawBStripAsButtonChromaEx(HDC hdc, int x, int y,
	int dx, int dy, HBSTRIP hbstrip, int ibmp, int dbsab, BOOL fChroma);

/* Compression/Decompression routines for bitmaps */
MSOAPI_(void) MsoUncompressLZW(const BYTE *pIn, _Out_bytecap_(cbOut) BYTE *pOut, int cbIn, int cbOut);
MSOAPI_(int) MsoCompressLZW(const BYTE *pIn, _Out_bytecap_(cbOutMax) BYTE *pOut, int cbIn, int cbOutMax);
...
/* Client side XSL files */
#define msorestypeXSL 226
