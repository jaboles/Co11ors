...
/****************************************************************************
	Defines the IMsoComponent2 interface

   IMsoComponent2 is an extension of IMsoComponent that allows clients to
   participate in message processing while waiting on handles.

   If your registered component implements IMsoComponent2 and calls
   ...
   Note: IMsoComponent2 derives from IMsoComponent
   Note: IMsoComponent2 must match IOleComponent2 from vsshell90.idl
****************************************************************************/
#undef  INTERFACE
#define INTERFACE  IMsoComponent2

DECLARE_INTERFACE_IID_(IMsoComponent2, IMsoComponent, "{ED0751FC-D772-4d1d-88FC-0C1AA275391B}")
	{
	BEGIN_MSOINTERFACE
    ...
    MSOMETHOD_(HRESULT, GetWaitHandlesAndTimeout) (THIS_
		HANDLE aHandles[64 /*MAXIMUM_WAIT_OBJECTS*/],
		UINT *pnHandles,
		DWORD *pdwTimeout,
		void *pvLoopData
		) PURE;
    ...
    // timeout is reset.
    MSOMETHOD_(HRESULT, OnHandleSignaled) (THIS_
		UINT nHandle,
		void *pvLoopData,
    ...
    // Otherwise, GetWaitHandleAndTimeout is called again and the cumulative
    // timeout is reset.
    MSOMETHOD_(HRESULT, OnTimeout) (THIS_
		void *pvLoopData,
		VARIANT_BOOL *pfContinue
		) PURE;
	};
...
DECLARE_INTERFACE_(IMsoStdComponentMgr, IUnknown)
	...
	/* Called by host just before it enters the 'wait mode' resulting from a
		call to WaitMessage, GetMessage, or MsgWaitForMultipleObjects(Ex).  Such a
...
/****************************************************************************
	Defines the IMsoStdComponentMgr2 interface

	IMsoStdComponentMgr2 is an extension of IMsoStdComponentMgr that supports
	IOleComponent2 components. These components support message loops which
	wait on handles.
****************************************************************************/
#undef  INTERFACE
#define INTERFACE  IMsoStdComponentMgr2

DECLARE_INTERFACE_IID_(IMsoStdComponentMgr2, IMsoStdComponentMgr, "{DA13A46D-71D5-4fff-AB49-78EBBF283429}")
	{
	BEGIN_MSOINTERFACE

	/* Called by host to get wait handle parameters from the current
		component. This is used only when the component implements IOleComponent2
		and the loop reason is either oleloopModelessFormWithHandles or
		oleloopModalFormWithHandles */
	MSOMETHOD_(void, GetWaitHandlesAndTimeout) (THIS_ HANDLE aHandles[64], UINT* pnHandles, DWORD* pdwTimeout) PURE;

	/* Called by host to indicate that the wait handle timed out.
		Returns TRUE to continue the loop, FALS if the loop should
		terminate. */
	MSOMETHOD_(BOOL, OnMsgWaitTimeout) (THIS) PURE;

	/* Called by host to indicate that one of the wait handles
		was signaled. Returns TRUE to continue the loop. FALSE
		if the loop should terminate. */
	MSOMETHOD_(BOOL, OnMsgWaitHandleSignaled) (THIS_ UINT nHandle) PURE;
	};
...
DECLARE_INTERFACE_(IMsoSimpleRecorder, IUnknown)
{
	...
	MSOMETHOD_(BOOL, FGetContext)(THIS_ int msosrctx, void *pv, _Out_z_cap_(cch) WCHAR *wz, int cch) PURE;
	...
	MSOMETHOD_(BOOL, FRecordLine)(THIS_ const WCHAR *wz) PURE;
