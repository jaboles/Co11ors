...
struct _dlg;
typedef struct _dlg **	HDLG;		// A near handle. 
...
#define hdlgtNull	((struct _dlgt * *) NULL)
...
#define tmmFocus         ((TMM)0x0024)   // Show yourself with/without focus.
...
// PHarring: Jan 2006. Added support to test flags on HDLG from components outside SDM (Toolbars, specifically)
MSOAPIX_(BOOL) FSdmTestHdlgFdlg(HDLG, FDLG);
...
typedef UCBK_SDM (SDM_CALLBACK * PFN_CTRL)(TMM, VOID *, UINT_SDM, UCBK_SDM, UCBK_SDM, TMC, UCBK_SDM);
#define pfnCtrlNull	((PFN_CTRL)0)

typedef UCBK_SDM (SDM_CALLBACK * PFN_PIC)(TMM, SDMP *, UINT_SDM, UCBK_SDM, UCBK_SDM, TMC, UCBK_SDM);
typedef UCBK_SDM (SDM_CALLBACK * PFN_PARSE)(TMM, WCHAR *, UINT_SDM, VOID **, BARG_SDM, TMC, UCBK_SDM);
typedef UCBK_SDM (SDM_CALLBACK * PFN_LISTBOX)(TMM, WCHAR *, UINT_SDM, ILBE_SDM, UCBK_SDM, TMC, UCBK_SDM);
typedef UCBK_SDM (SDM_CALLBACK * PFN_RENDER)(TMM, RDS *, UINT_SDM, FTMS, FTMS, TMC, UCBK_SDM);
typedef PFN_PARSE	PFN_FORMAT;

// Autocomplete call back
// Third Argument(WCHAR *) is the string to be autocompleted
// Fourth Argument (UINT_SDM) buffer size of third argument
// Fifth Argument(WCHAR *) is the return string by the function
// Sixth Argument (UINT_SDM) buffer size of fifth argument
// Return TRUE if autocompleting should occur
// Return FALSE if no changes(i.e. autocompletion) should occur
typedef BOOL_SDM (SDM_CALLBACK * PFN_AUTOCOMPLETE)(void *, TMC, WCHAR *, UINT_SDM, WCHAR *, UINT_SDM);
...
MSOAPI_(BOOL) FGetSDMHideUnderlines(HWND hwnd);
MSOAPI_(BOOL) FGetSDMSetFocusBecauseOfClick(void);
#if 0 //$[VSMSO]
MSOAPI_(BOOL) MsoFDlgIsWorkpane(HDLG hdlg);
MSOAPI_(void) MsoProtectSDMInWorkpane(void);
MSOAPI_(void) MsoUnprotectSDMInWorkpane(void);
MSOAPI_(void) MsoNotifySdmOfFocus(TMC tmc, BOOL fFocus);
MSOAPI_(SDM_PFNPEEKMESSAGE) LpfnMsoSetSdmPeekMessageWrap(SDM_PFNPEEKMESSAGE lpfn, int fUnicode);
MSOAPI_(SDM_PFNGETMESSAGE) LpfnMsoSetSdmGetMessageWrap(SDM_PFNGETMESSAGE lpfn, int fUnicode);
#endif //$[VSMSO]
...
BOOL_SDM SDM_CALLBACK MsoFSdmReallocPpv(SB_SDM sb, VOID **ppv, UINT_SDM cb);
...
typedef LBC  (LBOX_CALLBACK * LPFNCOMP)(UINT_SDM, VOID * *, VOID * *);
#ifndef	lpfncompNull
#define	lpfncompNull	((LPFNCOMP)NULL)
#endif	//!lpfncompNull
...
typedef struct _sdi
	...
	HINSTANCE hinstCur;		// Current application instance. 
	HINSTANCE hinstPrev;		// Previous application instance. 
...
typedef	struct _dlgh
	{
	struct _dlgt * * hdlgt; // Dialog Template. 
...
#define	hwndDlgClient	hwndDlg		// I know its gross, but it saves a lot
	...
	BOOL_SDM fHideUnderlines;
	PT_SDM ptClick;     // Mouse position of click for QuickTip
	} DLGH;				// Public part of DLG structure.
...
typedef struct _tmw
	...
	BYTE bCharSet;
#endif
	HIMC himcOld;		//Original himc assosiated to edit contorol.
...
MSOAPIX_(BOOL) MsoSDMFValidCurDlg();
