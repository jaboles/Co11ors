...
DECLARE_INTERFACE(IMsoSignature)
{
	...
	// this simply check whether the signature has been verified and look good
	// it is used by the client app to update the title bar and the status bar. If this
	// returns FALSE, it doesn't mean the signature is invalid, only that we don't
	// know its status.
	MSOMETHOD_(BOOL, FVerified)(THIS) PURE;
};
...
DECLARE_INTERFACE(IMsoSignatureSet)
{
	...
	// For example:
	//     dwIndex = 0;
	//     while (piss->FGetSignature(dwIndex, &pis))
	//        {
	//        if (!pis->FValid())
	...
	MSOMETHOD(HrHandsOffStorage2)(THIS) PURE;
};
#endif // MSO_NO_INTERFACES
...
enum
	...
	msodigevtSSVerified,         // fired when all the signatures in the signature set has been verified
							 // this helps app decide when to refresh the title bar etc
	msodigevtMax
	};
...
-------------------------------------------------------------------- HAILIU -*/
MSOAPI_(void) MsoGetSignatrueStmName(LPWSTR wz, DWORD cchMax);
