...
#define msofurlEscapeXMLChars           0x00400000  // client wants XML characters escaped.  Currently just "&".
#define msofurlNoExtension              0x00800000  // don't treat the '.' character as the extension delimiter
#define msofurlAllowEscapedServer       0x01000000  // allow the server component to include escape sequences
...
#define msofurldfKeepShellPathGuid      0x0010  // if this is a shell path, don't convert the ::{GUID} to display form
#define msofurldfUnescapeUTF8           0x0020  // unescape UTF8 escape sequences
...
#define msofurluNoUI               0x0004  // don't allow UI to be raised (such as the ftp login dialog)
#define msofurluOverwriteIfExist   0x0008
#define msofurluThickets           0x0010  // upload thicket files too
...
#define msofurldAllowUI           0x0002  // allow UI to be raised
#define msofurldAddOffVerHeader   0x0004  // add the Office-Version header to the download request

/* URLSP flags, passed to IMsoUrl::HrIsSafeProtocol. */
#define msofurlspAllowMinimum     0x0000  // the minimum list of safe protocols (http, https, mailto, ftp, and news) - same as no flag
#define msofurlspAllowFile        0x0001  // allow the file protocol
#define msofurlspAllowSTS         0x0002  // allow additional protocols from the STS list
#define msofurlspRequireSafeFile  0x0004  // forces file protocol to be local and not UNC to be valid

#define msofurlspUseMinimumSet      msofurlspAllowMinimum
#define msofurlspIncludeSafeFiles   (msofurlspAllowFile | msofurlspRequireSafeFile)
#define msofurlspIncludeFiles       (msofurlspAllowFile)
#define msofurlspIncludeSTS         (msofurlspAllowFile | msofurlspAllowSTS)
#define msofurlspUseMaximumSet      msofurlspIncludeSTS
...
DECLARE_INTERFACE_(IMsoUrl, IUnknown)
{
	...
	//    WzCanonicalForm.
	MSOMETHOD (HrGetDisplayForm) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf, DWORD grfurldf) CONST_METHOD PURE;
	MSOMETHOD (HrGetCanonicalForm) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetCustomForm) (THIS_ DWORD grfurlc, __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf, DWORD grfurlcf) CONST_METHOD PURE;
	...
	MSOMETHOD (HrGetScheme) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetAuthority) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetUserName) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetPassword) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetServer) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetPort) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetPath) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetDir) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetFileLeaf) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetFileName) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetFileExt) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetQuery) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	MSOMETHOD (HrGetFragment) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	...
	MSOMETHOD (HrGetLocalPath) (THIS_ __zcount(pcchBuf) WCHAR *wzBuf, int *pcchBuf) CONST_METHOD PURE;
	...
	// Returns true if the url is a "::{GUID}" url.
	MSOMETHOD_(BOOL, FIsShellPath) (THIS) CONST_METHOD PURE;

	// GetInternetPort - returns the internet port for this url,
	//    either given explicitly in the url or the default port
	//    for the protocol.  Returns INTERNET_INVALID_PORT_NUMBER
	//    on error computing the port.
	MSOMETHOD_(INTERNET_PORT, GetInternetPort) (THIS) CONST_METHOD PURE;

	// HrIsSafeProtocol - returns S_OK if a protocol is safe to execute
	// This function takes a flag indicating the groups of protocols to
	// allow.
	//
	// At the minimum level it only allows the protocols defined
	// in the SaferHtml spec (http, https, news, ftp, mailto).
	//
	// At the next highest it allows the file protocol (no UNC paths), then
	// the unrestricted file protocol.
	//
	// Finally at the current highest level it allows the list defined by STS
	//
	// The function returns S_OK if the protocol is valid and anything else if it 
	// isn't or an error has occurred (S_FALSE is not valid and other error codes
	// indicate other error conditions).
	//
	MSOMETHOD_(HRESULT, HrIsSafeProtocol) (THIS_ DWORD grfurlsp) CONST_METHOD PURE;
};
...
#define IMsoUrl_FIsShellPath(this) \
 	( (this)->lpVtbl->FIsShellPath(this) )
#define IMsoUrl_GetInternetPort(this) \
 	( (this)->lpVtbl->GetInternetPort(this) )
#define IMsoUrl_HrIsSafeProtocol(this, grfurlsp) \
 	( (this)->lpVtbl->HrIsSafeProtocol(this, grfurlsp) )
#endif // __cplusplus
...
MSOAPI_(HRESULT) MsoHrCreateUrlFromCanonicalUrl(interface IMsoUrl **ppiurl,
...
MSOAPI_(HRESULT) MsoHrSetUrlFromEscapedUTF8(const WCHAR *wzUrl,
	IMsoUrl *pimu);
MSOAPI_(HRESULT) MsoHrGetEscapedUTF8FromUrl(IMsoUrl *pimu,
	IMsoString **ppims);


//
// Check hyperlink protocol for safeness
//
MSOAPI_(HRESULT) MsoHrIsSafeProtocol(const WCHAR *wzUrl, DWORD grfurlsp, BOOL fTranslate);
...
MSOAPI_(WCHAR*) MsoWzNeuterUrlToPreventScriptAttack(const WCHAR* wzUrl);
MSOAPI_(HRESULT) MsoHrDownloadWithTimeout(IMsoUrl* purl, _Out_cap_(*pcchPath) WCHAR* wzPath, int* pcchPath, 
							    				DWORD grfurld, DWORD dwTimeout);
...
// OPEN ISSUES:
// 
// 1.  We don't handle "http://server.com" & "http://www.server.com"
