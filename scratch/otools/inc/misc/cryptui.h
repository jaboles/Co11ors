...
//
typedef HRESULT (__stdcall * PFNCMMCCALLBACK)(int *lNotifyHandle, LPARAM param);

typedef struct tagCRYPTUI_MMCCALLBACK_STRUCT {
    PFNCMMCCALLBACK pfnCallback;    // the address of MMCPropertyChangeNotify()
    int *         lNotifyHandle;  // the lNotifyHandle passed to MMCPropertyChangeNotify()
...
//  dwFlags            Optional:    Flags, Can be set to the following:
...
//CRYPTUI_WIZ_NO_SEARCH_CA is only valid for CryptUIWizCertRequest API
//the wizard will not search for a certificate authority in the network when needed
#define     CRYPTUI_WIZ_NO_SEARCH_CA                    0x0002
...
typedef struct _CRYPTUI_WIZ_CERT_REQUEST_INFO
{
    ...
    LPCWSTR             pwszCALocation;         //Required  if CRYPTUI_WIZ_NO_SEARCH_CA and CRYPTUI_WIZ_NO_UI 
                                                //          are both set in dwFlags; Or if CRYPTUI_WIZ_NO_SEARCH_CA
                                                //          is set in dwFlags and dwPurpose == CRYPTUI_WIZ_CERT_RENEW
                                                //Optional  Otherwise
                                                //          The machine name of the Certiviate Authority (CA)
    LPCWSTR             pwszCAName;             //Required  if CRYPTUI_WIZ_NO_SEARCH_CA and CRYPTUI_WIZ_NO_UI
                                                //          are both set in dwFlags   Or if CRYPTUI_WIZ_NO_SEARCH_CA
                                                //          is set in dwFlags and dwPurpose == CRYPTUI_WIZ_CERT_RENEW                       
                                                //Optional  Otherwise                                        
...
//      If CRYPTUI_WIZ_NO_SEARCH_CA is set in dwFlags, the wizard will not
//      search for available certificate authoritys (CA) on the newwork.
//      The caller has to supply the pwszCALocation and pwszCAname in
//      pCertRequestInfo struct for no ui option. 
