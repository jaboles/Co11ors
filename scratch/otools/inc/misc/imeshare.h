...
extern "C"{
#endif
BOOL	IMECDECL FInitIMEShare();
void	IMECDECL CustomizeIMEShare();
void	IMECDECL EndIMEShare();
...
BOOL	IMECDECL FRefreshStyle();

//substitution
BOOL	IMECDECL FSupportSty(UINT sty, UINT styAltered);

//style handling                            
const IMESTYLE * IMECDECL PIMEStyleFromAttr(const UINT attr);
const IMECOLORSTY * IMECDECL PColorStyleTextFromIMEStyle(const IMESTYLE * pimestyle);
const IMECOLORSTY * IMECDECL PColorStyleBackFromIMEStyle(const IMESTYLE * pimestyle);
...
BOOL	IMECDECL FBoldIMEStyle(const IMESTYLE *pimestyle);
BOOL	IMECDECL FItalicIMEStyle(const IMESTYLE *pimestyle);
BOOL	IMECDECL FUlIMEStyle(const IMESTYLE *pimestyle);
UINT	IMECDECL IdUlIMEStyle(const IMESTYLE *pimestyle);

//color handling
BOOL	IMECDECL FWinIMEColorStyle(const IMECOLORSTY *pcolorstyle);
BOOL	IMECDECL FFundamentalIMEColorStyle(const IMECOLORSTY *pcolorstyle);
BOOL	IMECDECL FRGBIMEColorStyle(const IMECOLORSTY *pcolorstyle);
BOOL	IMECDECL FSpecialIMEColorStyle(const IMECOLORSTY *pcolorstyle);
BOOL	IMECDECL FSpecialTextIMEColorStyle(const IMECOLORSTY *pcolorstyle);
BOOL	IMECDECL FSpecialWindowIMEColorStyle(const IMECOLORSTY *pcolorstyle);
COLORREF	IMECDECL RGBFromIMEColorStyle(const IMECOLORSTY *pcolorstyle);

#ifdef IMESHARE_CPL
UINT	IMECDECL IdSpecialFromIMEColorStyle(const IMECOLORSTY *pcolorstyle);
UINT	IMECDECL IdWinFromIMEColorStyle(const IMECOLORSTY *pcolorstyle);
UINT	IMECDECL IdFundamentalFromIMEColorStyle(const IMECOLORSTY *pcolorstyle);
GRFSTY	IMECDECL GrfStyIMEStyle(const IMESTYLE *pimestyle);
BOOL	IMECDECL FGetIMEStyleAttr(IMESTYLE *pimestyle, const UINT attr);
BOOL	IMECDECL FSetIMEStyleAttr(const IMESTYLE *pimestyle, const UINT attr);
BOOL	IMECDECL FSetIMEColorStyle(UINT attr, BOOL fTextCol, UINT id, DWORD col);
BOOL	IMECDECL FSetIMEStyle(const UINT attr, BOOL fBold, BOOL fItalic, BOOL fUl, UINT idUl);
BOOL	IMECDECL FSaveIMEShareSetting(void);
#endif // IMESHARE_CPL
#ifdef __cplusplus
}
#endif
